...
describe('String Expectations', function (): void {
    describe('String Pattern Methods', function (): void {
        test('toStartWith() accepts strings with matching prefix', function (): void {
            expect(fn (): Expectation => assertExpect('hello world')->toStartWith('hello'))->not->toThrow(Throwable::class);
            expect(fn (): Expectation => assertExpect('test')->toStartWith('te'))->not->toThrow(Throwable::class);
            expect(fn (): Expectation => assertExpect('PHP')->toStartWith('P'))->not->toThrow(Throwable::class);
        });

        test('toStartWith() rejects strings without matching prefix', function (): void {
            expect(fn (): Expectation => assertExpect('hello world')->toStartWith('world'))
                ->toThrow(InvalidArgumentException::class);
        });

        test('toEndWith() accepts strings with matching suffix', function (): void {
            expect(fn (): Expectation => assertExpect('hello world')->toEndWith('world'))->not->toThrow(Throwable::class);
            expect(fn (): Expectation => assertExpect('test')->toEndWith('st'))->not->toThrow(Throwable::class);
            expect(fn (): Expectation => assertExpect('PHP')->toEndWith('P'))->not->toThrow(Throwable::class);
        });

        test('toEndWith() rejects strings without matching suffix', function (): void {
            expect(fn (): Expectation => assertExpect('hello world')->toEndWith('hello'))
                ->toThrow(InvalidArgumentException::class);
        });

        test('toMatch() accepts strings matching regex', function (): void {
            expect(fn (): Expectation => assertExpect('test@example.com')->toMatch('/^.+@.+\..+$/'))->not->toThrow(Throwable::class);
            expect(fn (): Expectation => assertExpect('hello123')->toMatch('/^[a-z]+\d+$/'))->not->toThrow(Throwable::class);
            expect(fn (): Expectation => assertExpect('PHP')->toMatch('/^[A-Z]+$/'))->not->toThrow(Throwable::class);
        });

        test('toMatch() rejects strings not matching regex', function (): void {
            expect(fn (): Expectation => assertExpect('invalid-email')->toMatch('/^.+@.+\..+$/'))
                ->toThrow(InvalidArgumentException::class);
        });
        ...
        test('toHaveLength() accepts strings with exact length', function (): void {
            ...
            expect(fn (): Expectation => assertExpect('PHP')->toHaveLength(3))->not->toThrow(Throwable::class);
        });

        test('toHaveLength() rejects strings with different length', function (): void {
            expect(fn (): Expectation => assertExpect('hello')->toHaveLength(4))
                ->toThrow(InvalidArgumentException::class);
        });

        test('toContain() accepts strings containing substring', function (): void {
            expect(fn (): Expectation => assertExpect('hello world')->toContain('world'))->not->toThrow(Throwable::class);
            expect(fn (): Expectation => assertExpect('hello world')->toContain('hello'))->not->toThrow(Throwable::class);
            expect(fn (): Expectation => assertExpect('hello world')->toContain('o w'))->not->toThrow(Throwable::class);
        });

        test('toContain() rejects strings not containing substring', function (): void {
            expect(fn (): Expectation => assertExpect('hello world')->toContain('foo'))
                ->toThrow(InvalidArgumentException::class);
        });
    });
    ...
    describe('Chaining String Methods', function (): void {
        test('can chain multiple string assertions', function (): void {
            expect(fn (): Expectation => assertExpect('hello world')
                ->toBeString()
                ->toStartWith('hello')
                ->toEndWith('world')
                ->toContain('o w')
